...
public class JcrMappingTemplate extends JcrTemplate implements JcrMappingOperations {
    ...
    protected ObjectContentManager createPersistenceManager(Session session) throws RepositoryException,
            JcrMappingException {
        return new JcrObjectContentManagerImpl(session, mapper);
    }
    ...
    public void removeById(final java.lang.String id) {
        execute(new JcrMappingCallback<Void>() {
            @Override
            public Void doInJcrMapping(ObjectContentManager manager) throws JcrMappingException {
                Object object = manager.getObjectByUuid(id);
    ...
    public void removeByQuery(final Query query) {
        execute(new JcrMappingCallback<Void>() {
            @Override
            public Void doInJcrMapping(ObjectContentManager manager) throws JcrMappingException {
    ...
    public Object getObjectByUuid(final java.lang.String uuid) {
        return execute(new JcrMappingCallback<Object>() {
            @Override
            public Object doInJcrMapping(ObjectContentManager manager) throws JcrMappingException {
    ...
    public Collection getObjects(final Query query,long limit,long offset) {
        return execute(new JcrMappingCallback<Collection<?>>() {
            @Override
            public Collection<?> doInJcrMapping(ObjectContentManager manager) throws JcrMappingException {
            	if(manager instanceof JcrObjectContentManagerImpl){
            		return ((JcrObjectContentManagerImpl)manager).getObjects(query, limit, offset);
            	}
            	return manager.getObjects(query);
            }
        }, true);
