...
loginJs.indexs = {
		...
		var $parents = $(".m-log-box"),
			...
			$li.on("click",function(){
				if($(this).hasClass("item2")){
					$.ajax({
						url:ctx+"/getQrcodelt.do",
						type:"get",
						success:function(uuid){
							$("#qrcode").attr("src",ctx+"/getUrlQrcode.do?uuid="+uuid);
							$("#qrcode").attr("code",uuid);
							$("#qrparam input[name='qrcltId']").val(uuid);
						}
					})
					if(!connected){
						connect();
	...
	diyCheckbox : function(){
		...
		$parents.on("click",function(){
			if($checkboxIco.hasClass("z-slt")){
				...
				$("input[name='rememberMe']").remove();
			}else {
				$checkboxIco.addClass("z-slt");
				$checkboxIco.append("<input type='hidden' name='rememberMe' value='true'/>");
			}
		})
		this.diySelect();
	},
	...
	formValidate:function(){
		if($("input[name='username']").val().trim()==''){
			this.accountPopupHint({txt : "请输入正确的账号！"});
			return false;
		}
		if($("input[name='password']").val()==''){
			this.passwordPopupHint({txt : "请输入正确的密码！"});
			return false;
		}
		return true;
	}
