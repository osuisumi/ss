...
public class AuthenticationExceptionHandler {
    ...
    static {
        ...
        DEFAULT_ERROR_LIST.add(com.haoyu.sip.login.auth.AccountDisabledException.class);
        DEFAULT_ERROR_LIST.add(com.haoyu.sip.login.auth.InvalidLoginLocationException.class);
        DEFAULT_ERROR_LIST.add(com.haoyu.sip.login.auth.InvalidLoginTimeException.class);
        DEFAULT_ERROR_LIST.add(com.haoyu.sip.login.auth.IncorrectPasswordException.class);
    }
            ...
            for (final Class<? extends Exception> kind : this.errors) {
                for (final Class<? extends Exception> handlerError : e.getHandlerErrors().values()) {
                    if (handlerError != null && handlerError.equals(kind)) {
                        ...
                        if(handlerError.getSimpleName().equals("AccountNotFoundException")){
                        	 messageContext.addMessage(new MessageBuilder().error().source("username").code(messageCode).build());
                        }else if(handlerError.getSimpleName().equals("IncorrectPasswordException")){
                       	 	messageContext.addMessage(new MessageBuilder().error().source("password").code(messageCode).build());
                        }else{
