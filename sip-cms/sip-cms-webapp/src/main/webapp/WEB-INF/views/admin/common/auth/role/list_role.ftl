			...
			<script type="text/javascript">
					...
					function editForm(){
						...
						if(value!=''){
				            ...
				            type: 2,
				            title: '编辑角色',
				            content: "${ctx}/auth_roles/"+value+"/edit",
							area: [600,500],
				            zIndex: 19999,
				            yes: {
					            close: true,
					...
					function deleteRole(){
						...
						if(value!=''){
							confirm('确认要删除选中记录吗？',function(r){
								$.ajaxDelete('${ctx}/auth_roles/'+value, "", function(){
									$("#${formId}").submit();
								});
							});
						} 
					...
					function editRolePermission(){
						var value = getCheckedboxValues('${formId}','checkboxId',1);
						if(value!=''){
							window.location.href="${ctx}/auth_roles/"+value+"/editRolePermission?menuTreeId=${menuTreeId!}"
						}
					}
					...
					function editRoleMenu(){
						var value = getCheckedboxValues('${formId}','checkboxId',1);
						if(value!=''){
							window.location.href="${ctx}/auth_roles/"+value+"/editRoleMenu?menuTreeId=${menuTreeId!}"
						}
					}
			</script>
            </@authRolesDirective>
