...
<@lo.layout jsArray=jsArray>
		<div class="mis-index-wrap">
                <div class="mis-mod">
                    <div class="mis-mod-tt">
                        <h2 class="tt t1"><span>栏目信息</span></h2>
                    </div>
	...
	function beforeEditName(treeId, treeNode) {
	    ...
	    mylayerFn.open({
	            ...
	            type: 2,
	            title: '修改栏目',
	            content: '${ctx}/cms_channels/'+treeNode.id+"/edit",
				area: [600,600],
	            zIndex: 19999
	        });
	...
	function beforeRemove(treeId, treeNode) {
	    ...
	    if(treeNode.isParent){
	    	...
	    	confirm("确认删除 " + treeNode.name + " 吗？",function(){
	    		$.ajaxDelete("${ctx}/cms_channels/"+treeNode.id,"",function(data){
	...
	function addHoverDom(treeId, treeNode) {
	    ...
	    var addStr = "<span class='button add' id='addBtn_" + treeNode.tId
	        + "' title='新增栏目' onfocus='this.blur();'></span>";
	    sObj.after(addStr);
	    ...
	    if (btn) btn.bind("click", function(){
	        ...
	        mylayerFn.open({
	            ...
	            type: 2,
	            title: '新增栏目',
	            content: '${ctx}/cms_channels/create?pid='+treeNode.id,
				area: [600,600],
	            zIndex: 19999
	        });
