<#macro form id="saveForm" method="post" action="" onsubmit="true" buttons=[] saveCallback="" isLayer=false>
	<form id="${id}" method="${method}" action="${action}">
		 <#nested/>
		 <#if method=='put'||method="PUT"||method=='post'||method="POST">
			 <div class="mis-btn-row indent1">
	               <button  type="button" onclick="if(${onsubmit}){saveForm('${id}'<#if (saveCallback?? )&& saveCallback!=''>,${saveCallback}</#if>)}"  class="mis-btn mis-main-btn"><i class="mis-save-ico"></i>保存</button>
