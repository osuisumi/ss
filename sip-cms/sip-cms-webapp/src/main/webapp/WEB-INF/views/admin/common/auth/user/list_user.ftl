...
<@lo.layout>
		...
		<@authUsersDirective page=param_page!"1" size=param_size!"10" realName=param_realName!"">
		<div class="mis-index-wrap">
				<#import "../../include/form.ftl" as cf>
				<@cf.form id="${formId}" action="${ctx}/auth_users/list" method="get">				
                <div class="mis-mod">
                        ...
                        <div class="mis-table-mod">
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="mis-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><input type="checkbox" name="checkboxId" value="${authUser.id}"></td>
			...
			<script type="text/javascript">
					...
					function editForm(){
						...
						if(value!=''){
				            ...
				            type: 2,
				            title: '编辑用户',
				            content: "${ctx}/auth_users/"+value+"/edit",
							area: [600,500],
				            zIndex: 19999,
				            yes: {
					            close: true,
					...
					function deleteUser(){
						...
						if(value!=''){
							confirm('确认要删除选中记录吗？',function(r){
								$.ajaxDelete('${ctx}/auth_users/'+value, "", function(){
									$("#${formId}").submit();
								});
							});
						} 
					...
					function editUserRole(){
						var value = getCheckedboxValues('${formId}','checkboxId',1);
						if(value!=''){
							window.location.href="${ctx}/auth_users/"+value+"/editUserRole?menuTreeId=${menuTreeId!}"
						}
					}
